...
| --- | --- | --- | --- |
| Systems mechanism | Memory-bound temporal state materialization to compute-side GPU work through differentiation schedule and gradient kernel shaping. | Tiled scalar reduction was 0.098x while CUTLASS GEMM gradient was 34.14x; the matched optimized path was 128.75x memory-cheaper and 6.78x faster on the NVIDIA L4 task. | This is not a claim about all SNN workloads, all frameworks, neuromorphic hardware, or accuracy SOTA. NVIDIA L4 is not NVIDIA L40. |
